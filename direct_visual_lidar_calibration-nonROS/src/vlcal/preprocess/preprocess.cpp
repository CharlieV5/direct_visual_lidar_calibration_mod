...
namespace vlcal
{
  ...
  ;
    ...
    for (int i = 0; i < bag_filenames.size(); i++)
    {
      ...
      std::transform(points->points, points->points + points->size(), ply.vertices.begin(), [](const Eigen::Vector4d &p)
                     { return p.cast<float>().head<3>(); });
...
#pragma omp parallel for
    for (int i = 0; i < bag_filenames.size(); i++)
    {
      ...
      cv::imwrite(dst_path + "/" + bag_name + "_lidar_intensities.png", intensities); // 灰度图
      cv::imwrite(dst_path + "/" + bag_name + "_lidar_indices.png", indices_8uc4);    // 掩膜图，标记灰度图有点云的区域
    }

    //
    std::vector<std::string> bag_names(bag_filenames);
    std::transform(bag_filenames.begin(), bag_filenames.end(), bag_names.begin(), [](const auto &path)
      ...
      const std::string &image_topic)
  {
    ...
    if (camera_model != "auto")
    {
      ...
      else
      {
        ...
        std::transform(intrinsic_tokens.begin(), intrinsic_tokens.end(), intrinsics.begin(), [](const auto &token)
                       { return std::stod(token); });
        std::transform(distortion_tokens.begin(), distortion_tokens.end(), distortion_coeffs.begin(), [](const auto &token)
                       { return std::stod(token); });
      }
      ...
      const int num_threads)
  {
    ...
    while (true)
    {
      ...
      points = filter(points, [](const Eigen::Vector4d &p)
