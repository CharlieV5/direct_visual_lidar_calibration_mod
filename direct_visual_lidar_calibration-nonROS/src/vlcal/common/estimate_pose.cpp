...
namespace vlcal
{
      ...
      std::deque<bool> *inliers)
  {
    ...
    const double error_thresh_sq = std::pow(m_params.ransac_error_thresh, 2);
...
#pragma omp parallel for
    for (int i = 0; i < m_params.ransac_iterations; i++)
    {
...
#pragma omp critical
      ...
      const Eigen::Isometry3d &init_T_camera_lidar)
  {
    ...
    for (const auto &[pt_2d, pt_3d] : correspondences)
    {
      ...
      auto loss = new ceres::CauchyLoss(m_params.robust_kernel_width);
