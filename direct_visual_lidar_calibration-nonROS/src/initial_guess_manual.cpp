...
namespace vlcal
{
  ...
  public:
    ...
    void onSave()
    {
      if (T_lidar_camera_gizmo == nullptr)
        return;

      // // 保存控制点
      if (picking != nullptr && picking->correspondences.size() > 0)
      {
        std::ofstream ctrl_ofs(this->data_path + "/points.txt");
        if (ctrl_ofs)
        {
          for (const auto &correspondence : picking->correspondences)
          {
            ctrl_ofs << correspondence.first.x() << " " 
                     << correspondence.first.y() << " " 
                     << correspondence.second.x() << " " 
                     << correspondence.second.y() << " " 
                     << correspondence.second.z() << std::endl;
          }
          ctrl_ofs.close();
        }
      }
    ...
    void spin()
    {
      ...
      auto viewer = guik::LightViewer::instance();
      T_lidar_camera_gizmo = std::make_unique<guik::ModelControl>("T_lidar_camera", init_T_lidar_camera.matrix().cast<float>());
  ...
  private:
    ...
    std::unique_ptr<guik::ModelControl> T_lidar_camera_gizmo;
  };
